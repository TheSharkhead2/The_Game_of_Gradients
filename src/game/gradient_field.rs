...
use crate::game::constants::{
    BASE_ARROW_SCALE, EXPECTED_MAX_ARROW_SCALE, FIELD_SCALE, NUM_ARROWS_X, NUM_ARROWS_Y,
    VERTICAL_WINDOW_HEIGHT,
};
...
impl Gradient {
    ...
    pub fn x(&self, x: f32, y: f32) -> f32 {
        if self.x_functions.len() == 0 {
            // if no x functions currently in gradient, evaulate to 0
            return 0.;
        } else {
            let mut x_value = self.x_functions[0].2(x, y); // evaulate the first x function
            for function in self.x_functions.iter().skip(1) {
                // iterate through all other x functions
                match function.1 {
                    // match operation to combine with previous functions
    ...
    pub fn y(&self, x: f32, y: f32) -> f32 {
        if self.y_functions.len() == 0 {
            // if no y functions currently in gradient, evaulate to 0
            return 0.;
        } else {
            let mut y_value = self.y_functions[0].2(x, y); // evaulate the first y function
            for function in self.y_functions.iter().skip(1) {
                // iterate through all other y functions
                match function.1 {
                    // match operation to combine with previous functions
    ...
    pub fn add_x_function(
        &mut self,
        function_id: u32,
        operation: GradientOperation,
        function: fn(f32, f32) -> f32,
        function_string: String,
    ) {
        self.x_functions
            .push((function_id, operation, function, function_string)); // add function to x functions
    }

    /// Add a new y function to the gradient
    pub fn add_y_function(
        &mut self,
        function_id: u32,
        operation: GradientOperation,
        function: fn(f32, f32) -> f32,
        ...
        self.y_functions
            .push((function_id, operation, function, function_string)); // add function to y functions
    }
    ...
    pub fn remove_x_function(&mut self, function_id: u32) {
        ...
        match function_pos {
            Some(pos) => {
                self.x_functions.remove(*pos);
            } // if function found, remove it from vector
            None => (),
        }
    }
    ...
    pub fn x_text(&self) -> String {
        if self.x_functions.len() == 0 {
            // if no x functions currently in gradient, return 0
            ...
            for function in self.x_functions.iter().skip(1) {
                // iterate through all other x functions
                match function.1 {
                    // how functions are combined depends on the operation
                    GradientOperation::Add => {
                        x_text = format!("{} + {}", x_text, function.3);
                    } // add function to previous function
                    GradientOperation::Multiply => {
                        x_text = format!("({}) * {}", x_text, function.3)
                    } // with multiplication, need to add parenthesis
                }
            }
    ...
    pub fn y_text(&self) -> String {
        if self.y_functions.len() == 0 {
            // if no y functions currently in gradient, return 0
            return String::from("0");
        } else {
            ...
            for function in self.y_functions.iter().skip(1) {
                ...
                match function.1 {
                    // how functions are combined depends on the operation
                    GradientOperation::Add => {
                        y_text = format!("{} + {}", y_text, function.3);
                    } // add function to previous function
                    GradientOperation::Multiply => {
                        y_text = format!("({}) * {}", y_text, function.3)
                    } // with multiplication, need to add parenthesis
...
fn update_gradient_arrows(
    ...
    // window information: https://stackoverflow.com/questions/63652767/how-with-bevy-can-you-get-and-set-window-information-after-creation
    let window = wnds.single();

    let pixel_to_coord_scale = window.resolution.height() / VERTICAL_WINDOW_HEIGHT; // scale factor to convert from world coordinates to pixels

    let window_width = window.resolution.width() / pixel_to_coord_scale; // width of the window in world coordinates
    ...
    for (mut gradient_arrow, _sprite, _transform) in gradient_arrows.iter_mut() {
        ...
        gradient_arrow.x =
            (x_number as f32) * window_width / ((NUM_ARROWS_X as f32) - 1.) - window_width / 2.; // get the x coordinate of the arrow
        gradient_arrow.y = (y_number as f32) * VERTICAL_WINDOW_HEIGHT
            / ((NUM_ARROWS_Y as f32) - 1.)
            - VERTICAL_WINDOW_HEIGHT / 2.; // get the y coordinate of the arrow

        gradient_arrow.scale =
            BASE_ARROW_SCALE * gradient.magnitude(gradient_arrow.x, gradient_arrow.y); // get the scaling factor for the arrow

        if max_magnitude < gradient_arrow.scale {
            // if the current magnitude is greater than the current max, update the max
            max_magnitude = gradient_arrow.scale;
        }
    }

    for (mut gradient_arrow, mut sprite, mut transform) in gradient_arrows.iter_mut() {
        let scale = (EXPECTED_MAX_ARROW_SCALE * BASE_ARROW_SCALE)
            * (gradient_arrow.scale / (0.6 * max_magnitude)).powf(1. / 7.);

        gradient_arrow.angle = gradient
            .y(gradient_arrow.x, gradient_arrow.y)
            .atan2(gradient.x(gradient_arrow.x, gradient_arrow.y))
            - 0.25 * PI; // get the angle of the arrow

        sprite.color = Color::hsla(
            240. - gradient_arrow.scale / (max_magnitude) * 240.,
            1.,
            0.8,
            1.,
        );
...
impl Plugin for GradientArrowPlugin {
    ...
    fn build(&self, app: &mut App) {
        app.add_systems(Startup, (initialize_gradient, spawn_gradient_arrows));
        app.add_systems(Update, update_gradient_arrows);
        app.add_state::<GradientOperationState>(); // state for when adding new functions to gradient... operation to add with
    }
}
